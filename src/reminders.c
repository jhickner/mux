...
static time_t parse_at(const char *s) {
    ...
    for (int i = 0; fmts[i]; i++) {
        ...
        tm.tm_isdst = -1;
        if (strptime(s, fmts[i], &tm)) return mktime(&tm);
    }
    return (time_t)-1;
}
...
static void parse_hhmm(const char *s, int *h, int *m) {
    *h = 8; *m = 0;                       /* default 08:00 */
    if (s) sscanf(s, "%d:%d", h, m);
}
...
static int dow_num(const char *s) {
    ...
    char b[4] = {0};
    for (int i = 0; i < 3 && s[i]; i++) b[i] = (char)tolower((unsigned char)s[i]);
    for (int i = 0; i < 7; i++) if (!strcmp(b, nm[i])) return i;
    if (s[0] >= '0' && s[0] <= '6' && s[1] == '\0') return s[0] - '0';
    return -1;
}
...
static int day_matches(cJSON *rule, const struct tm *t) {
    ...
    if (!kind) return 0;
    if (!strcmp(kind, "daily")) return 1;
    if (!strcmp(kind, "weekdays")) return t->tm_wday >= 1 && t->tm_wday <= 5;
    if (!strcmp(kind, "weekly")) {
        cJSON *days = cJSON_GetObjectItem(rule, "days"), *d;
        cJSON_ArrayForEach(d, days)
            if (dow_num(cJSON_GetStringValue(d)) == t->tm_wday) return 1;
        return 0;
    }
    if (!strcmp(kind, "nth_weekday")) {
        ...
        if (t->tm_wday != dow) return 0;
        if (nth == -1) {                 /* last <dow> of the month */
            struct tm x = *t; x.tm_mday += 7; x.tm_isdst = -1;
            time_t tt = mktime(&x); struct tm y; localtime_r(&tt, &y);
...
static time_t next_occurrence(cJSON *rule, time_t after) {
    ...
    base.tm_hour = hh; base.tm_min = mm; base.tm_sec = 0;
    for (int d = 0; d < 420; d++) {      /* ~14 months of lookahead */
        ...
        if (day_matches(rule, &day)) return cand;
    }
    return (time_t)-1;
}
...
static int reschedule(cJSON *o, time_t fired_ts, time_t now) {
    ...
    if (rule && cJSON_IsObject(rule)) {
        ...
        char b[32]; fmt_at(next, b, sizeof b);
        cJSON_DeleteItemFromObject(o, "at");
        cJSON_AddStringToObject(o, "at", b);
        return 1;
    }
...
int reminders_pop_due(time_t now, char *out, size_t n) {
    ...
    for (char *p = buf; *p && nobj < 512; ) {
        char *nl = strchr(p, '\n');
        if (nl) *nl = '\0';
        if (*p) { cJSON *o = cJSON_Parse(p); if (o) objs[nobj++] = o; }
        if (!nl) break;
        p = nl + 1;
    }
    free(buf);
    ...
    for (int i = 0; i < nobj; i++) ats[i] = effective_at(objs[i], now, &dirty);

    /* Fire the earliest that's due. */
    int fired = -1; time_t fired_ts = 0;
    for (int i = 0; i < nobj; i++)
        if (ats[i] != (time_t)-1 && ats[i] <= now && (fired < 0 || ats[i] < fired_ts)) {
            fired = i; fired_ts = ats[i];
        }
    ...
    for (int i = 0; i < nobj; i++) if (objs[i]) cJSON_Delete(objs[i]);
    return result;
}
